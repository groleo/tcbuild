# This file declares functions to install the newlib C library
# Copyright 2007 Yann E. MORIN
# Licensed under the GPL v2. See COPYING in the root of this package
PKG_NAME=newlib
PKG_SRC="${PKG_NAME}-${CT_LIBC_VERSION}"
PKG_URL="ftp://sources.redhat.com/pub/newlib/"
...
# This functions installs newlib's headers
do_libc_headers() {
    ...
    # Simply copy files until newlib has the ablity to build out-of-tree
    CT_DoLog EXTRA "Copying sources to build dir"
    { cd "${CT_SRC_DIR}/${PKG_SRC}"; tar cf - .; } |tar xf -

    # Retrieve the config file
    CT_DoExecLog ALL cp "${CT_LIBC_NEWLIB_CONFIG_FILE}" .config

    # newlib uses the CROSS environment variable as a prefix to the
...
do_libc() {
    ...
    # Retrieve the config file
    cp "${CT_LIBC_NEWLIB_CONFIG_FILE}" .config

    # newlib uses the CROSS environment variable as a prefix to the compiler
    # tools to use.  The newly built tools should be in our path, so we need
    # only give the correct name for them.
    # Note about CFLAGS: In newlib, CFLAGS are generated by Rules.mak,
    ...
    CT_DoExecLog ALL                                    \
    make ${CT_LIBC_NEWLIB_VERBOSITY} \
		${CT_LIBC_NEWLIB_PARALLEL:+${PARALLELMFLAGS}}  \
		CROSS=${CT_TARGET}-                            \
		PREFIX="${CT_SYSROOT_DIR}/"    \
		STRIPTOOL=true                                 \
         all
    ...
    if [ "${CT_LIBC_NEWLIB_BUILD_CROSS_LDD}" = "y" ]; then
...
mungeeglibcConfig() {
    src_config_file="$1"
    dst_config_file="$2"
    munge_file="${CT_BUILD_DIR}/munge-newlib-config.sed"
...
ENDSED
    ...
    # Accomodate for old and new newlib versions, where the
...
ENDSED
    ...
    # Accomodate for old and new newlib version, where the
    # way to select between hard/soft float has changed
    case "${CT_ARCH_FLOAT_HW},${CT_ARCH_FLOAT_SW}" in
        y,) cat >>"${munge_file}" <<-ENDSED
s/^[^_]*(HAS_FPU).*/\\1=y/
s/.*(NEWLIB_HAS_FPU).*/\\1=y/
ENDSED
            ;;
        ,y) cat >>"${munge_file}" <<-ENDSED
s/^[^_]*(HAS_FPU).*/\\# \\1 is not set/
s/.*(NEWLIB_HAS_FPU).*/# \\1 is not set/
ENDSED
            ;;
    esac
    ...
    # Some (old) versions of newlib use KERNEL_SOURCE (which is _wrong_), and
...
s/^NEWLIB_DOWNLOAD_PREGENERATED_LOCALE=y/\\# NEWLIB_DOWNLOAD_PREGENERATED_LOCALE is not set/
ENDSED
    ...
    # arrangements.  Note that having the newlib Makefile download the
    # pregenerated locales is not compatible with crosstool; besides,
    # crosstool downloads them as part of getandpatch.sh.
    if [ "${CT_LIBC_NEWLIB_LOCALES}" = "y" ] ; then
       cat >>"${munge_file}" <<-ENDSED
s/^# NEWLIB_HAS_LOCALE is not set/NEWLIB_HAS_LOCALE=y\\nNEWLIB_PREGENERATED_LOCALE_DATA=y\\n\\# NEWLIB_DOWNLOAD_PREGENERATED_LOCALE_DATA is not set\\n\\# NEWLIB_HAS_XLOCALE is not
ENDSED
    fi
...
s/^# NEWLIB_CTOR_DTOR is not set/NEWLIB_CTOR_DTOR=y/
# Add these three lines when doing C++?
s/^# NEWLIB_HAS_WCHAR is not set/NEWLIB_HAS_WCHAR=y/
#s/^# NEWLIB_HAS_LOCALE is not set/NEWLIB_HAS_LOCALE=y\\nNEWLIB_PREGENERATED_LOCALE_DATA=y\\n\\# NEWLIB_DOWNLOAD_PREGENERATED_LOCALE_DATA is not set\\n\\# NEWLIB_HAS_XLOCALE is not set/
s/^# NEWLIB_HAS_GNU_GETOPT is not set/NEWLIB_HAS_GNU_GETOPT=y/
ENDSED
    fi
...
ENDSED
    ...
    case "${CT_LIBC_NEWLIB_DEBUG_LEVEL}" in
      0)
        cat >>"${munge_file}" <<-ENDSED
...
s/^NEWLIB_MALLOC_DEBUGGING=y/# NEWLIB_MALLOC_DEBUGGING is not set/
ENDSED
        ;;
      1)
        cat >>"${munge_file}" <<-ENDSED
...
s/^# NEWLIB_MALLOC_DEBUGGING is not set/NEWLIB_MALLOC_DEBUGGING=y/
ENDSED
        ;;
    esac
