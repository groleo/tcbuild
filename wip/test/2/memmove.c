...
void* memmove(void*dest, const void*src,size_t len)
{
	...
	"Lmemmove_FM:\n"
	"	cmpi.l #48,%0			/*  len ? 48( 12regs * 4bytes)	*/\n"
	"	jlt Lmemmove_F4			/*  IF (len-48) < 0 GOTO 2f; ELSE*/\n"
	"	movem.l (%0),%%d1-%%d7/%%a2-%%a6	/*  move *src to regs		*/\n"
	"	movem.l %%d1-%%d7/%%a2-%%a6,(%1)	/*  7cfe move regs to *dest	*/\n"
	"	lea.l (48,%0),%0		/*  src  = src + 48		*/\n"
	"	lea.l (48,%1),%1		/*  dest = dest+ 48			*/\n"
	"	subi.l #48,%2			/*  n -= 48			*/\n"
	"	jra Lmemmove_FM		/*  one more iteration		*/\n"
	"\n"
	"Lmemmove_F4:\n"
	"	cmpi.l #4, %2			/*  Copy in 4 bytes chunks?	*/\n"
	"	jlt Lmemmove_F2			/*  No, THEN GOTO 3f	*/\n"
	"	move.l (%0),%%d1		/*  move *src to reg		*/\n"
	"	move.l %%d1,(%1)		/*  move reg to *dest		*/\n"
	"	lea.l (4,%0),%0		/*  src  = src + 4		*/\n"
	"	lea.l (4,%1),%1		/*  dest = dest+ 4		*/\n"
	"	subq.l #4,%2			/*  len -= 4			*/\n"
	"	jra Lmemmove_F4			/*  one more iteration		*/\n"
	"Lmemmove_F2:\n"
	"	cmpi.l #2, %2			/*  Copy in 2 bytes chunks?	*/\n"
	"	jlt Lmemmove_F1			/*  No, THEN GOTO 4f	*/\n"
	"	move.w (%0),%%d1		/*  move *src to reg;		*/\n"
