...
int testMemmoveAlign(const char*ref,size_t SIZE)
{
	...
	for (j=1;j<SIZE/2;++j)
	{
		char *src=getBuffer(SIZE+10);
		  memmove(src+SIZE/2, src, j);
		rv=differ(src+SIZE/2, src, j) ;
		if ( rv )
		{
			printf("Error 1\n----------------------------------------\n");
			continue;
	...
	{
		char *src=getBuffer(SIZE+10);
		  memmove(src+SIZE/2, src+j, SIZE/2);
		rv=differ(src+SIZE/2, ref+j, SIZE/2) ;
		if ( rv )
		{
			printf("Error 1\n----------------------------------------\n");
			continue;
	...
	for ( j=1;j<SIZE/2;++j)
	{
		...
		//  |-------------j+++++++++++|
		//  <------SIZE--------------->
		printf("%s:%p,%p,%i,%i,%i\n",__FUNCTION__,src+j,src+1,SIZE-j,j,min(j-1,SIZE-j-1));
		printBuf(src,SIZE);

		rv=differ( src+j+1+min(j,SIZE/2), ref+j+1+min(j,SIZE/2), SIZE-min(j,SIZE/2));
		rv=differ( src+j+1+min(j,SIZE/2), ref+j+1+min(j,SIZE/2))
		if ( rv ) printf("Error 3\n----------------------------------------\n");
		free(src);
		printf("\n");
	}
...
//	TIME( ITERATIONS, testMemmove(dst, src, TEST_SZ) );
	...
	TIME( ITERATIONS, testMemmoveAlign( src, 60) );
