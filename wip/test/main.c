...
int testLmbenchFcp(char * dst, char* src, size_t SIZE)
{
	int i,rv,j;
...
int testBcopy(char * dst, char* src, size_t SIZE)
{
	int i,rv,j;

	//printf("%s:%i ",__FUNCTION__,SIZE);
	my_bcopy(src,dst,SIZE);
	rv=differ(dst,src,SIZE) ;

	//printf(" ok\n");
	return 0;
}
int testMemcpy(char * dst, char* src, size_t SIZE)
{
	int i,rv,j;
...
#define ITERATIONS 1
	...
	TIME( ITERATIONS, testMemcpy(dst, src,TEST_SZ) );
